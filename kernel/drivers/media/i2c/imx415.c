...
#define MIPI_FREQ_1188M        1188000000 //Consti
...
struct imx415_mode {
	...
	// in rpi driver, this is called line_length_pix.
	// same behaviour here and in rpi driver - fixed, calc once in same way (value-H=hts)
...
static const struct imx415_mode supported_modes[] = {
    ...
    /*tested, gives 3 lines, then black image{
            ...
            //.height = 1+12+(1080)+2+1,
            .height =1+12+8+1080+8+2+1,
            .max_fps = {
                    .numerator =     10000,
                    .denominator = 1800000,
            },
...
static int imx415_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_VBLANK:
		...
		// consti10: too small vts is not allowed, set by isp anyways sometimes ?!
		/*if(vts<imx415->cur_mode->vts_def){
            dev_dbg(&client->dev, "Got V4L2_CID_VBLANK, but value is too low. wanted vts: %d | min: %d\n",
                    vts, imx415->cur_mode->vts_def);
            // sensor seems to ignore it anyways ?
            //break;
		}*/
		/*
		...
		ret = imx415_write_reg(imx415->client, IMX415_VMAX_L,
				       IMX415_REG_VALUE_08BIT,
				       IMX415_FETCH_VTS_L(vts));
		ret |= imx415_write_reg(imx415->client, IMX415_VMAX_M,
				       IMX415_REG_VALUE_08BIT,
