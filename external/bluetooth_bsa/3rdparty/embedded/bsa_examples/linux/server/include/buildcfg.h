#ifndef BUILDCFG_H
#define BUILDCFG_H
#define HCITHIN_INCLUDED   FALSE
#define HCISU_H4_INCLUDED   TRUE
#define HCILP_INCLUDED   FALSE
#define HCILP_SLEEP_MODE   1
#define H4IBSS_INCLUDED   FALSE
#define BSA_SLEEP_PROC_IF   FALSE
#define RFKILL_BT_POWER   FALSE
#define BSA_UART_SEND_BREAK_FOR_4374   FALSE
#define HCISU_H4_BAUDRATE   USERIAL_BAUD_115200
#define BTM_SCO_HCI_INCLUDED   TRUE
#define BTM_WBS_INCLUDED   TRUE
#define BSA_SERIAL_DEV_NAME   "/dev/btusb0"
#define BSA_CHANGE_STARTUP_BAUDRATE   USERIAL_BAUD_3M
#define BTA_CHANGE_PRM_BAUDRATE   USERIAL_BAUD_3M
#define BTSNOOPDISP_INCLUDED   TRUE
#define BSA_DM_START_TIMEOUT   20000
#define BSA_AFH_BEHAVIOR_DEBUG   FALSE
#define BT_BRCM_VS_INCLUDED   TRUE
#define BT_BRCM_I2SPCM_ENABLE_FLAG    FALSE
#define BT_BRCM_I2SPCM_SLAVE_ROLE   FALSE
#define GAP_INCLUDED   TRUE
#define BTA_GATT_INCLUDED   TRUE
#define BLE_INCLUDED   TRUE
#define ATT_INCLUDED   TRUE
#define GATT_SERVER_ENABLED   TRUE
#define GATT_CLIENT_ENABLED   TRUE
#define FORCE_ALL_ADV_SCAN_RSP_REPORT   TRUE
#define MESH_BREDRLE_COEX_SUP   TRUE
#define BTA_GATTC_CL_MAX   4
#define BSA_BLE_CLIENT_MAX   (BTA_GATTC_CL_MAX - 1)
#define SMP_INCLUDED   TRUE
#define SMP_LE_SC_INCLUDED   TRUE
#define BTA_GATT_DEBUG   TRUE
#define BTA_HH_LE_INCLUDED   TRUE
#define BTA_GATTC_NOTIF_REG_MAX   10
#define BTA_HH_LE_RPT_MAX   10
#define BTA_HH_GATT_MTU   255
#define BLE_CON_CPLT_MISSING_FW_BUG   TRUE
#define BLE_2M_PHY_INCLUDED   TRUE
#define L2CAP_INCLUDED   TRUE
#define L2CAP_COMPRESSION_INCLUDED   FALSE
#define L2CAP_HOST_FLOW_CTRL         FALSE
#define L2CAP_FCR_INCLUDED   TRUE
#define MAX_L2CAP_CHANNELS   20
#define MAX_L2CAP_CLIENTS   12
#define L2CAP_DESIRED_LINK_ROLE   HCI_ROLE_MASTER
#define L2CAP_UCD_INCLUDED   TRUE
#define L2CAP_UCD_IDLE_TIMEOUT   20000
#define SDP_DI_INCLUDED   TRUE
#define SDP_RAW_DATA_INCLUDED   TRUE
#define SDP_MAX_PAD_LEN   1024
#define SDP_MTU_SIZE   510
#define RFCOMM_INCLUDED   TRUE
#define MAX_BD_CONNECTIONS   4
#define OBX_INCLUDED   TRUE
#define OBX_SERVER_INCLUDED   TRUE
#define OBX_CLIENT_INCLUDED   TRUE
#define OBX_MD5_INCLUDED      TRUE
#define GOEP_INCLUDED   TRUE
#define GOEP_FS_INCLUDED   TRUE
#define BTA_DG_INCLUDED   TRUE
#define SPP_INCLUDED   TRUE
#define DUN_INCLUDED   TRUE
#define BTA_AG_INCLUDED   TRUE
#define BTA_HS_INCLUDED   TRUE
#define HSP2_INCLUDED   FALSE
#define HFP_INCLUDED   FALSE
#define OPP_INCLUDED   TRUE
#define OPP_SERVER_INCLUDED   TRUE
#define OPP_CLIENT_INCLUDED   FALSE
#define BTA_OP_INCLUDED   TRUE
#define BTA_OPS_INCLUDED   TRUE
#define BTA_OPC_INCLUDED   TRUE
#define BTA_OPS_DEBUG   FALSE
#define FTP_INCLUDED   TRUE
#define FTP_SERVER_INCLUDED   TRUE
#define FTP_CLIENT_INCLUDED   FALSE
#define BTA_FT_INCLUDED   TRUE
#define BTA_FTS_INCLUDED   TRUE
#define BTA_FTC_INCLUDED   TRUE
#define BIP_INCLUDED   TRUE
#define BIP_INITR_INCLUDED   TRUE
#define BIP_RSPDR_INCLUDED   TRUE
#define BTA_BI_INCLUDED   TRUE
#define BIP_PRINTING_INCLUDED   TRUE
#define BIP_PUSH_INCLUDED   FALSE
#define BIP_PULL_INCLUDED   FALSE
#define BIP_ARCHIVE_INCLUDED   FALSE
#define BIP_CAMERA_INCLUDED   FALSE
#define BIP_DISPLAY_INCLUDED   FALSE
#define BPP_INCLUDED   FALSE
#define BPP_SND_INCLUDED   FALSE
#define HCRP_INCLUDED   FALSE
#define HCRP_CLIENT_INCLUDED   FALSE
#define HCRP_SERVER_INCLUDED   FALSE
#define BTA_PR_INCLUDED   FALSE
#define BTA_RC_INCLUDED   TRUE
#define AVRC_COVER_ART_INCLUDED   TRUE
#define CAC_OPEN_EVENT_STATUS_CHANGE   TRUE
#define AVCT_INCLUDED   TRUE
#define AVRC_INCLUDED   TRUE
#define AVDT_INCLUDED   TRUE
#define A2D_INCLUDED   TRUE
#define A2D_SBC_INCLUDED   TRUE
#define A2D_M12_INCLUDED   TRUE
#define A2D_M24_INCLUDED   TRUE
#define VDP_INCLUDED       TRUE
#define VDP_H263_INCLUDED   TRUE
#define VDP_MPEG_INCLUDED   TRUE
#define VDP_VEND_INCLUDED   TRUE
#define BTA_AV_INCLUDED   TRUE
#define BTA_AVK_INCLUDED   TRUE
#define BTA_AR_INCLUDED   TRUE
#define BTA_AV_CO_CP_SCMS_T   TRUE
#define BTA_AVK_CO_CP_SCMS_T   TRUE
#define BSA_AV_UIPC_BUFFER_SIZE   20001
#define BSA_AV_SBC_CAPS_SAMP_FREQ   A2D_SBC_IE_SAMP_FREQ_48 | A2D_SBC_IE_SAMP_FREQ_44
#define BSA_AV_SBC_DEFAULT_SAMP_FREQ   A2D_SBC_IE_SAMP_FREQ_48
#define BSA_AV_SBC_CH_MODE   A2D_SBC_IE_CH_MD_JOINT
#define BSA_AV_SBC_BLOCK_LEN   A2D_SBC_IE_BLOCKS_16
#define BSA_AV_SBC_NUM_SUBBAND   A2D_SBC_IE_SUBBAND_8
#define BSA_AV_SBC_ALLOC_MD   A2D_SBC_IE_ALLOC_MD_L
#define BSA_AV_SBC_MAX_BP   0x35
#define BSA_MEDIA_AV_FLOW_DEBUG   FALSE
#define BSA_MEDIA_BITRATE_STEP   10
#define BSA_AV_BITRATE_CONTROL_BY_USER   FALSE
#define AVDT_NUM_SEPS   6
#define AVDT_DEBUG   TRUE
#define BTA_AVK_AV_AUDIO_RELAY   TRUE
#define BSA_AVK_AV_AUDIO_RELAY   TRUE
#define BSA_AVK_M24_SUPPORT   FALSE
#define BAV_L2CAP_HDR_SZ   4
#define BSA_SV_AV_BAV_HCI_HEADER_SIZE   (BAV_L2CAP_HDR_SZ + 4)
#define BAV_L2CAP_CID   0x1234
#define BSA_SV_AV_BAV_SBC_SAMPLING_RATE   48000
#define BSA_SV_AV_BAV_SBC_NB_BLOCKS   12
#define BSA_BAV_SBC_RATE_LEVEL   220
#define BTM_CLB_SYNC_TRAIN_SERVICE_DATA   0x00
#define BTA_BAV_NB_RE_TX   1
#define BTM_CLB_SYNC_TRAIN_TIMEOUT   0xFFFE
#define BTA_BAV_TX_QUEUE_SIZE   1
#define BTA_BAV_NEED_ACLDATA_BEFORE_SYNCTRAIN   TRUE
#define BTA_BAV_KEEP_SYNCTRAIN_FOR_NEWSINK   TRUE
#define BTU_DUAL_STACK_BTC_CP_INCLUDED   TRUE
#define SBC_FOR_EMBEDDED_LINUX   TRUE
#define BLE_AUDIO_INCLUDED   FALSE
#define BTA_AC_INCLUDED   FALSE
#define BTA_CT_INCLUDED   FALSE
#define BTA_CG_INCLUDED   FALSE
#define CTP_INCLUDED   FALSE
#define TCS_INCLUDED   FALSE
#define ICP_INCLUDED   FALSE
#define ICA_INCLUDED   FALSE
#define BNEP_INCLUDED   TRUE
#define PAN_INCLUDED   TRUE
#define BTA_PAN_INCLUDED   TRUE
#define BNEP_MAX_PROT_FILTERS   10
#define BNEP_MAX_MULTI_FILTERS   10
#define HID_HOST_INCLUDED   TRUE
#define BTA_HH_DEBUG   TRUE
#define BTA_HH_INCLUDED   TRUE
#define HID_DEV_INCLUDED   TRUE
#define BTA_HD_INCLUDED   TRUE
#define HID_DEV_MTU_SIZE   800
#define HID_HOST_MAX_CONN_RETRY   1
#define BRR_HID_INCLUDED   FALSE
#define HIDH_UCD_INCLUDED   TRUE
#define BSA_HID_3DG_INCLUDED   FALSE
#define BSA_MIXED_MODE_INCLUDED   FALSE
#define HID_HOST_MTU   700
#define BSA_MESH_INCLUDED   TRUE
#define MESH_STACK_INCLUDED   TRUE
#define WICED_BT_MESH_MODEL_ONOFF_CLIENT_INCLUDED   TRUE
#define WICED_BT_MESH_MODEL_LEVEL_CLIENT_INCLUDED   TRUE
#define WICED_BT_MESH_MODEL_LIGHT_LIGHTNESS_CLIENT_INCLUDED   TRUE
#define WICED_BT_MESH_MODEL_DEFAULT_TRANSITION_TIME_CLIENT_INCLUDED   TRUE
#define WICED_BT_MESH_MODEL_SCENE_CLIENT_INCLUDED   TRUE
#define WICED_BT_MESH_MODEL_LIGHT_HSL_CLIENT_INCLUDED   TRUE
#define WICED_BT_MESH_MODEL_LIGHT_CTL_CLIENT_INCLUDED   TRUE
#define WICED_BT_MESH_MODEL_SENSOR_CLIENT_INCLUDED   TRUE
#define MESH_NVRAM_NOT_SUPPORTED   TRUE
#define _64_BIT_   TRUE
#define WICED_BT_MESH_TRACE_ENABLE   TRUE
#define WICED_BT_MESH_CORE_TRACE_ENABLE   TRUE
#define WICEDX_LINUX   TRUE
#define WICEDX   TRUE
#define WICEDX_MESH   TRUE
#define HCI_CONTROL   TRUE
#define CYW20719B1    TRUE
#define WICED_SDK_MAJOR_VER   0
#define WICED_SDK_MINOR_VER   0
#define WICED_SDK_REV_NUMBER   0
#define WICED_SDK_BUILD_NUMBER   0
#define POWER_CLASS   0
#define AES_ENC_PREKEYED   TRUE
#define MESH_CUST   xiaomi
#define SIM_ACCESS_INCLUDED   TRUE
#define SAP_SERVER_INCLUDED   TRUE
#define SAP_CLIENT_INCLUDED   TRUE
#define SAP_INCLUDED   FALSE
#define BTA_PBS_INCLUDED   TRUE
#define BTA_PBC_INCLUDED   TRUE
#define BTA_PBAP_1_2_SUPPORTED   TRUE
#define BTA_MCE_INCLUDED   TRUE
#define BTA_MAP_1_2_SUPPORTED   TRUE
#define BTA_SS_INCLUDED   FALSE
#define BTA_SC_INCLUDED   TRUE
#define BTA_SAC_INCLUDED   TRUE
#define BTA_FM_INCLUDED   TRUE
#define BTA_FMTX_INCLUDED   FALSE
#define BTA_FM_AUDIO_OUT_DAC   TRUE
#define BTA_FM_VOLUME_DEFAULT   50
#define BTA_HL_INCLUDED   TRUE
#define BTA_HL_DEBUG   TRUE
#define BTA_HL_DISC_SIZE   3000
#define SLIP_INCLUDED   FALSE
#define SLIP_STATIS_INCLUDED   FALSE
#define SLIP_SW_FLOW_CTRL   FALSE
#define BT_TRACE_SLIP   FALSE
#define SLIP_SLEEP_TO   5000
#define SLIP_HOST_SLIDING_WINDOW_SIZE   7
#define MCA_INCLUDED   TRUE
#define MCA_SYNC_INCLUDED   TRUE
#define MCA_NUM_DEPS   5
#define MCA_NUM_MDLS   5
#define L2CAP_EXTFEA_SUPPORTED_MASK   (L2CAP_EXTFEA_ENH_RETRANS | L2CAP_EXTFEA_STREAM_MODE | L2CAP_EXTFEA_NO_CRC )
#define BSA_3DS_INCLUDED   TRUE
#define BTA_3DS_INCLUDED   TRUE
#define BTA_3DS_DEBUG   TRUE
#define BSA_3DTV_INCLUDED   TRUE
#define BSA_MIP_HYBRID   FALSE
#define BSA_3DTV_MCAST   TRUE
#define BTA_GCE_INCLUDED   FALSE
#define BTA_BRCM_INCLUDED   TRUE
#define BTA_DM_INCLUDED   TRUE
#define BTA_FS_INCLUDED   TRUE
#define BTA_DI_INCLUDED   TRUE
#define BTA_EIR_CANNED_UUID_LIST   FALSE
#define GPS_INCLUDED   FALSE
#define BTA_DM_REMOTE_DEVICE_NAME_LENGTH   248
#define BSA_POWER_INCLUDED   TRUE
#define BSA_POWER_INQ_TX_RSP_INCLUDED   TRUE
#define BSA_TBFC_SCAN_INCLUDED   TRUE
#define BTM_INQ_DB_SIZE   20
#define BTM_SCO_INCLUDED   TRUE
#define BTM_LISBON_INCLUDED   TRUE
#define BTM_OOB_INCLUDED   FALSE
#define BTM_SEC_REMOVE_NOT_IN_USE   TRUE
#define BTM_MAX_REM_BD_NAME_LEN   100
#define BTM_DEFAULT_CONN_INTERVAL   800
#define PATCHRAM_INCLUDED   TRUE
#define PATCHRAM_DIR_INCLUDED   FALSE
#define BTM_APP_DEV_INIT   bta_sys_dev_init
#define BTM_TBFC_INCLUDED   TRUE
#define BTM_BI_DIR_TBFC_INCLUDED   FALSE
#define BTM_CLB_INCLUDED   TRUE
#define ROLE_CONFIG_DEFAULT   ROLE_CONFIG_NO_ROLE_SWITCH
#define BTM_SCO_PCM_INTF   FALSE
#define BTM_BLE_PRIVACY_SPT   TRUE
#define BTM_BLE_MAKE_FALSE_LL_PRIVACY   TRUE
#define BTM_SEC_REMOVE_NOT_IN_USE   TRUE
#define BTU_BTA_INCLUDED   TRUE
#define BTU_STACK_LITE_ENABLED   FALSE
#define BTU_DUAL_STACK_DEBUG   TRUE
#define BTU_DUAL_STACK_INCLUDED   TRUE
#define BTU_DUAL_STACK_BTC_INCLUDED   TRUE
#define BTU_DUAL_STACK_MM_INCLUDED   TRUE
#define BTU_HANDLE_RAW_HCI_ENABLED   TRUE
#define BTTRC_INCLUDED   FALSE
#define BTTRC_PARSER_INCLUDED   FALSE
#define MAX_TRACE_RAM_SIZE   10000
#define TRACE_TASK_INCLUDED   TRUE
#define BT_TRACE_PROTOCOL   TRUE
#define BT_USE_TRACES   TRUE
#define BT_TRACE_APPL   TRUE
#define BT_USE_TRACES_TIMESTAMP   TRUE
#define BT_USE_TRACES_SYSLOG   FALSE
#define BT_USE_TRACES_SYSLOG_MSG_SIZE   256
#define BSA_COLORED_TRACES   TRUE
#define BSA_INITIAL_TRACE_LEVEL   BT_TRACE_LEVEL_DEBUG
#define BTM_INITIAL_TRACE_LEVEL   BT_TRACE_LEVEL_DEBUG
#define L2CAP_INITIAL_TRACE_LEVEL   BT_TRACE_LEVEL_DEBUG
#define APPL_INITIAL_TRACE_LEVEL   BT_TRACE_LEVEL_DEBUG
#define AVRC_INITIAL_TRACE_LEVEL   BT_TRACE_LEVEL_DEBUG
#define PAN_INITIAL_TRACE_LEVEL   BT_TRACE_LEVEL_DEBUG
#define BNEP_INITIAL_TRACE_LEVEL   BT_TRACE_LEVEL_DEBUG
#define GKI_NUM_FIXED_BUF_POOLS   9
#define GKI_BUF3_MAX   200
#define GKI_BUF3_SIZE   4096
#define BRCM_GKI_INCLUDED   TRUE
#define GKI_USE_PTHREADS   TRUE
#define GKI_TIMER_LIST_NOPREEMPT   TRUE
#define UCODEC_TASK   3
#define BT_MEDIA_TASK   UCODEC_TASK
#define GKI_DEF_BUFPOOL_PERM_MASK   0xffe0
#define BSA_TM_INCLUDED   TRUE
#define BSA_NSA_INCLUDED   TRUE
#define BTAPP_INCLUDED   TRUE
#define BTA_CONFIG_PATH_LEN   500
#define BTA_CONFIG_FILE_LEN   300
#define BTA_DEVICES_PATH_LEN   500
#define BTA_DEVICES_FILE_LEN   300
#define BTA_FS_PATH_LEN   500
#define BTA_FS_FILE_LEN   300
#define BSA_HW_HOT_RESET   FALSE
#define BTA_DM_CO_INCLUDE_BTUI   FALSE
#define LINUX_OS   TRUE
#define FLASH_AVAILABLE   FALSE
#define BTA_DM_COD   {0x00, 0x00, 0x00}
#define AT91_MAIN_INCLUDED   FALSE
#define AT91_DRV_INCLUDED   FALSE
#define AT91_LIB_INCLUDED   FALSE
#define AT91_GKI_INCLUDED   FALSE
#define UNV_INCLUDED   FALSE
#define BBY_MAIN_INCLUDED   FALSE
#define BRCM_LIB_INCLUDED   TRUE
#define BRCM_DRV_INCLUDED   TRUE
#define MMI_INCLUDED   FALSE
#define BTA_JV_INCLUDED   FALSE
#define RPC_INCLUDED   FALSE
#define RSI_INCLUDED   TRUE
#define RPC_TRACE_ONLY   FALSE
#define APP_REQ_CFG   TRUE
#define BSA_MESH_ALT_BRANCH   TRUE
#define LOCAL_DEVICE_TTL   5
#define LOCAL_DEVICE_NET_TRANSMIT_COUNT   2
#define LOCAL_DEVICE_NET_TRANSMIT_INTERVAL   50
#define LOCAL_DEV_FDT_RELAY_STATE   1
#define LOCAL_DEV_FDT_RELAY_RETX_COUNT   2
#define LOCAL_DEV_FDT_RELAY_RETX_INTERVAL   50
#define LOCAL_DEV_FDT_RELAY_TX_COUNT   2
#define LOCAL_DEV_FDT_RELAY_TX_INTERVAL   50
#define LOWER_TRANSPORT_SEG_TRANS_TIMEOUT_MS   600
#define LOWER_TRANSPORT_ACK_TIMEOUT_MS   550
#define LOWER_TRANSPORT_RETRANS   3
#endif
