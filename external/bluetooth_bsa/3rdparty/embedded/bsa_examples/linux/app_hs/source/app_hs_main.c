...
void app_hs_display_main_menu(void)
{
    ...
    printf("    %d  => Send Battery Level Indicator\n", APP_HS_KEY_BL_IND);
    printf("    %d  => Quit\n", APP_HS_KEY_QUIT);
}
...
int main(int argc, char **argv)
{
    ...
    do
    {
        ...
        case APP_HS_KEY_CLOSE:
            ...
            app_hs_display_connections();
            connection_index = app_get_choice("\n");
            p_conn = app_hs_find_connection_by_index(connection_index);
            if(p_conn)
                app_hs_close(p_conn->handle);
            break;
        ...
        case APP_HS_KEY_PICKUP_CALL:
            ...
            p_conn = app_hs_find_connection_by_index(connection_index);
            if(p_conn)
                app_hs_answer_call(p_conn->handle);
            break;
        ...
        case APP_HS_KEY_RECORD:
        {
            ...
            app_hs_open_rec_file(APP_HS_SCO_IN_SOUND_FILE, sample_rate);
        }
        break;
        ...
        case APP_HS_KEY_PLAY:
            ...
            p_conn = app_hs_get_active_audio_conn();
            if(p_conn)
            {
                app_hs_play_file(p_conn->handle, APP_HS_SCO_OUT_SOUND_FILE);
                printf("Finished reading audio file...all data sent to SCO\n");
            }
            break;

        case APP_HS_KEY_BL_IND:
            /* example to send battery level indicator to AG */
            printf("Choose connection index\n");
            app_hs_display_connections();
            connection_index = app_get_choice("\n");
            p_conn = app_hs_find_connection_by_index(connection_index);
            battery_level = app_get_choice("Battery Level (0 - 100) :");
            if ((p_conn) && (battery_level >= 0) && (battery_level <= MAX_BATTERY_LEVEL))
                app_hs_send_bl_ind(p_conn->handle, (UINT8)battery_level);
            break;
