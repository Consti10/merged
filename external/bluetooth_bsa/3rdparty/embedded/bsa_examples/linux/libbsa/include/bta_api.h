...
#define BTA_BLE_APCF_COND_ADD    BTM_BLE_APCF_COND_ADD
#define BTA_BLE_APCF_COND_DELETE BTM_BLE_APCF_COND_DELETE
#define BTA_BLE_APCF_COND_CLEAR  BTM_BLE_APCF_COND_CLEAR
typedef tBTM_BLE_APCF_COND_OP tBTA_BLE_APCF_COND_OP;

typedef tBTM_BLE_APCF_FILT_INDEX tBTA_BLE_APCF_FILT_INDEX;

/* filter selection bit index  */
#define BTA_BLE_APCF_FILTER_SETTING BTM_BLE_APCF_FILTER_SETTING
#define BTA_BLE_APCF_ADDR_FILTER    BTM_BLE_APCF_ADDR_FILTER
#define BTA_BLE_APCF_SRVC_UUID      BTM_BLE_APCF_SRVC_UUID
#define BTA_BLE_APCF_SRVC_SOL_UUID  BTM_BLE_APCF_SRVC_SOL_UUID
#define BTA_BLE_APCF_LOCAL_NAME     BTM_BLE_APCF_LOCAL_NAME
#define BTA_BLE_APCF_MANU_DATA      BTM_BLE_APCF_MANU_DATA
#define BTA_BLE_APCF_SRVC_DATA      BTM_BLE_APCF_SRVC_DATA
typedef tBTM_BLE_APCF_COND_TYPE  tBTA_BLE_APCF_COND_TYPE;

#define BTA_BLE_APCF_BRDCAST_ADDR_FILT  BTM_BLE_APCF_BRDCAST_ADDR_FILT
#define BTA_BLE_APCF_SERV_DATA_CHG_FILT BTM_BLE_APCF_SERV_DATA_CHG_FILT
#define BTA_BLE_APCF_SERV_UUID          BTM_BLE_APCF_SERV_UUID
#define BTA_BLE_APCF_SERV_SOLC_UUID     BTM_BLE_APCF_SERV_SOLC_UUID
#define BTA_BLE_APCF_LOC_NAME_CHECK     BTM_BLE_APCF_LOC_NAME_CHECK
#define BTA_BLE_APCF_MANUF_NAME_CHECK   BTM_BLE_APCF_MANUF_NAME_CHECK
#define BTA_BLE_APCF_SERV_DATA_CHECK    BTM_BLE_APCF_SERV_DATA_CHECK
typedef tBTM_BLE_APCF_FEAT_SEL tBTA_BLE_APCF_FEAT_SEL;

#define BTA_BLE_APCF_LIST_LOGIC_OR     BTM_BLE_APCF_LIST_LOGIC_OR
#define BTA_BLE_APCF_LIST_LOGIC_AND    BTM_BLE_APCF_LIST_LOGIC_AND
typedef tBTM_BLE_APCF_LIST_LOGIC_TYPE tBTA_BLE_APCF_LIST_LOGIC_TYPE;

#define BTA_BLE_APCF_FILT_LOGIC_OR     BTM_BLE_APCF_FILT_LOGIC_OR
#define BTA_BLE_APCF_FILT_LOGIC_AND    BTM_BLE_APCF_FILT_LOGIC_AND
typedef tBTM_BLE_APCF_FILT_LOGIC_TYPE tBTA_BLE_APCF_FILT_LOGIC_TYPE;

typedef tBTM_BLE_APCF_RSSI_THRESHOLD  tBTA_BLE_APCF_RSSI_THRESHOLD;

typedef struct
{
    tBTA_BLE_APCF_FEAT_SEL        feat_seln;
    tBTA_BLE_APCF_LIST_LOGIC_TYPE logic_type;
    tBTA_BLE_APCF_FILT_LOGIC_TYPE filt_logic_type;
    tBTA_BLE_APCF_RSSI_THRESHOLD  rssi_high_thres;
}tBTA_BLE_APCF_FILT_PARAMS;
...
typedef union
{
      ...
      UINT32              uuid32_mask;
      UINT8               uuid128_mask[LEN_UUID_128];
}tBTA_BLE_APCF_COND_MASK;

typedef struct
{
    tBT_UUID                   uuid;           /* UUID condition */
    tBTA_BLE_APCF_COND_MASK    uuid_mask;      /* UUID condition mask, if NULL, match exact as UUID condition */
}tBTA_BLE_APCF_UUID_COND;

typedef struct
{
    UINT8                   data_len;       /* <= 29 bytes */
    UINT8                   *p_data;
}tBTA_BLE_APCF_LOCAL_NAME_COND;

typedef struct
{
    UINT8    data_len;      /* <= 29 bytes */
    UINT8    *p_data;       /* manufactuer/service data pattern to look for */
    UINT8    *p_data_mask;  /* manufactuer/service data matching mask, same length as
                               data pattern, set to all 0xff or NULL pointer, match exact the same data pattern */
}tBTA_BLE_APCF_DATA_COND;

typedef union
{
    tBLE_BD_ADDR                     target_addr;
    tBTA_BLE_APCF_LOCAL_NAME_COND    local_name; /* lcoal name filtering */
    tBTA_BLE_APCF_DATA_COND          data;       /* manufactuer/service data filtering */
    tBTA_BLE_APCF_UUID_COND          uuid;       /* service/solicitated UUID filtering */
    tBTA_BLE_APCF_FILT_PARAMS        filter_setting;
}tBTA_BLE_APCF_COND_PARAM;
