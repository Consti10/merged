 ...
 * @{ */
#define WICED_BT_MESH_FW_UPDATE_APPLY                       155  /**< Abort firmware update */
/** @} FW_UPDATE_EVENT */

/**
 * @anchor OBJ_TRANSFER_EVENT
 * @name Definition for messages exchanged between an app and Object Transfer Model
 * @{ */
#define WICED_BT_MESH_OBJ_TRANSFER_PHASE_STATUS             156  /**< Start obj transfer */
#define WICED_BT_MESH_OBJ_INFO_STATUS                       157  /**< Reply to get info message */
#define WICED_BT_MESH_OBJ_TRANSFER_START                    158  /**< Data tranfer started */
#define WICED_BT_MESH_OBJ_TRANSFER_DATA                     159  /**< Data chunk from the distributor */
#define WICED_BT_MESH_OBJ_TRANSFER_FINISH                   160  /**< Data transfer completed */
/** @} OBJ_TRANSFER_EVENT */
...
#define WICED_BT_MESH_MODEL_FW_UPDATE_SERVER \
    { MESH_COMPANY_ID_BT_SIG, WICED_BT_MESH_CORE_MODEL_ID_OBJ_TRANSFER_SRV, wiced_bt_mesh_model_obj_transfer_server_message_handler, NULL, NULL }, \
...
#define WICED_BT_MESH_MODEL_FW_DISTRIBUTION_SERVER \
    ...
    { MESH_COMPANY_ID_BT_SIG, WICED_BT_MESH_CORE_MODEL_ID_OBJ_TRANSFER_CLNT, wiced_bt_mesh_model_obj_transfer_client_message_handler, NULL, NULL }
...
#define WICED_BT_MESH_MODEL_OBJ_TRANSFER_CLIENT \
    { MESH_COMPANY_ID_BT_SIG, WICED_BT_MESH_CORE_MODEL_ID_OBJ_TRANSFER_CLNT, wiced_bt_mesh_model_obj_transfer_client_message_handler, NULL, NULL }
...
/**
 * Object Trasfer Prepare structure exchanged between the application and Object Transfer model
 */
typedef struct
{
    uint8_t obj_id[WICED_BT_MESH_OBJ_TRANSFER_OBJ_ID_LEN];  /**< Object ID if specified by upper level, otherwise set to all 0s */
} wiced_bt_mesh_obj_transfer_prepare_t;

/**
 * Object Trasfer Block Data structure exchanged between the application and Object Transfer model
 */
typedef struct
{
    uint32_t data_len;                              /**< Block data length */
    uint8_t  *p_data;                               /**< Pointer to data buffer */
} wiced_bt_mesh_obj_transfer_block_data_t;

/**
 * Object Trasfer Finish structure exchanged between the application and Object Transfer model
 */
typedef struct
{
    uint8_t obj_transfer_result;                    /**< Object transfer final result */
} wiced_bt_mesh_obj_transfer_finish_t;
...
#define WICED_BT_FIRMWARE_ID_LEN            8           /**< Currently we define 8 bytes firmware id for both mesh and OTA use TODO */
...
typedef struct
{
    ...
    uint8_t object_id[8];                   /**< Object ID   to be prepared */
...
#define MESH_MAX_FIRMWARE_UPDATE_OBJECT_LEN 64
    uint8_t object_id[MESH_MAX_FIRMWARE_UPDATE_OBJECT_LEN]; /**< Object ID */
...
/**
* Object Transfer Status Data structure exchanged between the application and Generic Level Model
*/


/* Object Transfer States */

typedef struct
{
    uint8_t     obj_transfer_phase;                     /**< Status of the Object Transfer Phase*/
}wiced_bt_mesh_obj_transfer_phase_state_t;


/* Object Transfer Capabilities States */
typedef struct
{
    ...
    uint8_t     max_chunk_size_log;                     /**< Max size chunk sze: 2^Max Chunk size Log */
} mesh_obj_transfer_capabilities_state_t;
...
{
    uint8_t     obj_transfer_phase;                     /**< Status of the Object Transfer Phase*/
    uint8_t     object_id[8];                           /**< Unique object identifier */
} wiced_bt_mesh_obj_transfer_phase_status_t;

typedef struct
{
    uint8_t     status;                                 /**< Status of the Object Transfer */
    uint8_t     object_id[8];                           /**< Unique object identifier */
    uint32_t    obj_size;                               /**<  size of the current object */
    uint8_t     cur_block_size_log;                     /**< current block size represented as log2(current block size) */
} wiced_bt_mesh_obj_transfer_status_t;

typedef struct
{
    uint8_t     object_id[8];                           /**< Unique object identifier */
    uint16_t    block_num;                              /**< Block numnber */
} wiced_bt_mesh_obj_transfer_block_get_t;

typedef struct
{
#define WICED_BT_MESH_OBJ_TRANSFER_SUCCESS      0
#define WICED_BT_MESH_OBJ_TRANSFER_FAILED       1
#define WICED_BT_MESH_OBJ_TRANSFER_ABORTED      2
    uint8_t     status;                                 /**< Object block status */
} wiced_bt_mesh_obj_transfer_complete_t;


typedef struct
{
    uint16_t    node_addr;                              /**< Address of the node which reported the info */
    uint8_t     min_block_size_log;                     /**< Minimum block size: 2^Max Block size Log */
    uint8_t     max_block_size_log;                     /**< Maximum block size: 2^Max Block size Log */
    uint16_t    max_chunk_number;                       /**< Supported maximum number of chunks in block */
    uint8_t     max_chunk_size_log;                     /**< Max size chunk sze: 2^Max Chunk size Log */
}wiced_bt_mesh_obj_transfer_information_status_data_t;

/**
 * @addtogroup  wiced_bt_mesh_obj_transfer_client   Mesh Object Transfer Client
 * @ingroup     wiced_bt_mesh_models
 *
 * The WICED Mesh Object Transfer Client module provides a way for an application to transfer object to Object Transfer Servers  in the mesh network
 *
 * TBD
 *
 * @{
 */
/**
 * \brief Object Transfer  callback is called by the Mesh Models library on receiving a message from the peer
 *
 * @param       event The event that the application should process (see @ref LEVEL_EVENT "Mesh Level Events")
 * @param       p_data Pointer to the level data
 *
 * @return      None
 */
typedef void(wiced_bt_mesh_obj_transfer_client_callback_t)(uint16_t event, void *p_data);

/**
 * \brief  Object Transfer Client Data callback is called by the Mesh Models library request data for chunk transfer
 *
 * @param       p_data Pointer to the chunk data
 * @param       offset offset into the object data
 * @param       data_len len of chunk data requested
 * @return      None
 */
typedef void(wiced_bt_mesh_obj_transfer_client_data_callback_t)(uint32_t offset, uint8_t *p_data, uint16_t data_len);


/**
* \brief  Object Transfer Client Message Handler
* \details The Mesh Core library calls this function for each message received.  The function returns WICED_TRUE if the message is destined for this specific model and successfully processed, and returns WICED_FALSE otherwise. Generic Level Server device.
* The function parses the message and if appropriate calls the parent back to perform functionality.
*
* @param       p_event Mesh event with information about received message.
...
* @return      WICED_TRUE if the message is for this company ID/Model/Element Index combination, WICED_FALSE otherwise.
*/
wiced_bool_t wiced_bt_mesh_model_obj_transfer_client_message_handler(wiced_bt_mesh_event_t *p_event, uint8_t *p_data, uint16_t data_len);

/**
 * \brief Object Transfer Client Module initialization
 * \details A layer that needs to utilize Object Transfer Client services shall call this function to register a callback which will be
 ...
 * @return      None
 */
void wiced_bt_mesh_model_obj_transfer_client_init(wiced_bt_mesh_obj_transfer_client_callback_t *p_callback, wiced_bt_mesh_obj_transfer_client_data_callback_t *p_data_callback);

/**
 * \brief Object Transfer Client Get Info
 * \details A layer that utilizing Object Transfer Client services can call this function to to inquire device capabilities of a specic node.
 * called on the execution of further function calls.
 *
 * @param       unicast_addr Address of the Object Transfer Server
 ...
 */
wiced_bool_t wiced_bt_mesh_object_transfer_client_get_info(uint16_t unicast_addr, uint16_t app_key_idx);

/**
 * \brief The application can call this function to send Get Object Transfer Phase Client message to the server.
 *
 * @param       unicast_addr Address of the Object Transfer Server
 * @param       app_key_idx App Key Index to be used to secure the message
 *
 ...
 */
wiced_bool_t wiced_bt_mesh_model_obj_transfer_client_phase_get(uint16_t unicast_addr, uint16_t app_key_idx);

/**
 * \brief Object Transfer Start
 * \details A client model above object transfer client or an application can call this function to start actual Object transfer.
 * The tranfer is done using the multicast to a group address.  The object is split into blocks which is then split into chunks.  The
 * group list is used by the object transfer client to identify which chunks of a block has not been received and retransmit them.
 *
 ...
 * @param       object_id           Object ID for the current transmission
 * @param       object_size         Size of the object to distribute
 * @param       block_size_log      Size of each block
 * @param       chunk_size          Sizeof of each chunk
 ...
 */
wiced_bool_t wiced_bt_mesh_model_obj_transfer_client_start(uint16_t group_addr, uint16_t *p_group_list, uint16_t group_size, uint8_t *p_object_id, uint32_t object_size, uint8_t block_size_log, uint16_t chunk_size, uint16_t app_key_idx);

/**
 * \brief Object Transfer Abort
 * \details A client model above object transfer client or an application can call this function to abort Object transfer.
 *
 * @param       object_id           Object ID for the current transmission
 *
 * @return      WICED_TRUE if distribution has been started successfully
 */
wiced_bool_t wiced_bt_mesh_model_obj_transfer_client_abort(uint8_t *p_object_id);

/**
 * \brief Object Transfer Client Get Status
 * \details A client model above object transfer client or an application can call this function to retrieve status of current transfer.
 ...
 * @return      WICED_TRUE if distribution has been started successfully
 */
wiced_bool_t wiced_bt_mesh_object_transfer_client_get_status(uint8_t *p_state, uint16_t *cur_block_num, uint16_t *p_total_blocks);
/* @} wiced_bt_mesh_object_transfer_client */
 ...
 * @return      None
 */
typedef void(wiced_bt_mesh_fw_update_server_callback_t)(uint16_t event, wiced_bt_mesh_event_t *p_event, void *p_data);
...
/**
 * @addtogroup  wiced_bt_mesh_obj_transfer_server   Mesh Object Transfer Server
 * @ingroup     wiced_bt_mesh_models
 *
 * The WICED Mesh Object Transfer Server module provides a way for an application to recevie Objects in the mesh network
 *
 * TBD
 *
 * @{
...
/*
 * \brief Object transfer server callback is called by the Mesh Models library on receiving a message from the peer
 *
 * @param       event   event The event that the application should process (see @ref FW_UPDATE_EVENT "Firmware Update Events")
 * @param       p_data  pointer to the object transfer data
 *
 * @return      None
 */
typedef void(wiced_bt_mesh_obj_transfer_server_callback_t)(uint16_t event, void *p_data);

/**
 * \brief Object Transfer Server Module initialization
 *
 * @param       element_idx     Device element to where model is used
 * @param       p_callback      The application callback function that will be executed by the mesh models library when application action is required, or when a reply for the application request has been received.
 * @param       is_provisioned  If TRUE, the application is being restarted after being provisioned or after a power loss. If FALSE the model cleans up NVRAM on startup.
 ...
 * @return      None
 */
void wiced_bt_mesh_model_obj_transfer_server_init(uint8_t element_idx, wiced_bt_mesh_obj_transfer_server_callback_t *p_callback, wiced_bool_t is_provisioned);

/**
 * \brief Object Transfer Server Message Handler
 * \details The Mesh Core library calls this function for each message received.  The function returns WICED_TRUE if the message is destined for this specific model and successfully processed, and returns WICED_FALSE otherwise.
 * The function parses the message and if appropriate calls the parent back to perform functionality.
 *
 * @param       p_event   Mesh event with information about received message.
 ...
 * @return      WICED_TRUE if the message is for this company ID/Model/Element Index combination, WICED_FALSE otherwise.
*/
wiced_bool_t wiced_bt_mesh_model_obj_transfer_server_message_handler(wiced_bt_mesh_event_t *p_event, uint8_t *p_data, uint16_t data_len);

/**
 * \brief Object Transfer Server prepare for transfer
 *
 * @param       element_idx Device element to where model is used
 * @param       p_data      Pointer to the data portion of the message
 *
 * @return      None
 */
void wiced_bt_mesh_model_obj_transfer_prepare(uint8_t element_idx, void *p_data);

/**
 * \brief Object Transfer Server abort transfer
 *
 * @param       element_idx Device element to where model is used
 *
 * @return      None
 */
void wiced_bt_mesh_model_obj_transfer_abort(uint8_t element_idx);
/* @} wiced_bt_mesh_obj_transfer_server */
