        ...
        tBSA_HH_REPORT_DATA *p_report_data)
{
    ...
    case APP_HH_GOOGLE_AUDIO_REPORT_ID0:
    case APP_HH_GOOGLE_AUDIO_REPORT_ID1:
    case APP_HH_GOOGLE_AUDIO_REPORT_ID2:
        app_hh_ble_msbc_decode(p_hh_dev, p_report_data->data,
           p_report_data->length);
        break;
#else
    case APP_HH_AUDIO_REPORT_ID:
        if((p_report_data->data[2] == mSBC_HEADER_SYNCWORD) &&
           (p_report_data->data[3] == 0x00) &&
           (p_report_data->data[4] == 0x00))
        {
            app_hh_ble_msbc_decode(p_hh_dev, p_report_data->data,
                                   p_report_data->length);
        }
        ...
        {
            app_hh_adpcm_decode(p_hh_dev, p_report_data->data,
                               p_report_data->length, app_hh_ble_audio_sink_cb.pkt_cnt);
        ...
        {
            app_hh_opus_decode(p_hh_dev, p_report_data->data,
                               p_report_data->length);
        }
#endif
...
static UINT16 app_hh_ble_msbc_dec_dump(SINT16 *s16PacketPtr, UINT16 u16NumberOfPCM)
{
    ...
    APP_DEBUG1("app_hh_ble_msbc_dec_dump (pcm16 = %d)\n", u16NumberOfPCM);
    APP_DUMP("data: ", p, 240);
#endif
...
#if defined(APP_HH_MSBC_DEBUG) && (APP_HH_MSBC_DEBUG == TRUE)
    APP_DEBUG1("app_hh_ble_msbc_dec_fill - u16NumOfBytes = %d\n", u16NumberOfBytes);
...
void app_hh_ble_msbc_decode(tAPP_HH_DEVICE *p_hh_dev, UINT8 *p_data, UINT16 length)
{
    ...
    /* Check report ID */
#if defined(GOOGLE_ALT_AUDIO_FORMAT) && (GOOGLE_ALT_AUDIO_FORMAT == TRUE)
    if ((p_data[0] != APP_HH_GOOGLE_AUDIO_REPORT_ID0) &&
        (p_data[0] != APP_HH_GOOGLE_AUDIO_REPORT_ID1) &&
        (p_data[0] != APP_HH_GOOGLE_AUDIO_REPORT_ID2))
#else
...
#if defined(GOOGLE_ALT_AUDIO_FORMAT) && (GOOGLE_ALT_AUDIO_FORMAT == TRUE)
    report_id = p_data[0];
    printf("report_id (0x%02X): \n", report_id);
    /*
     * For Google VoHOGP, the Audio Report format is:
     * APP_HH_GOOGLE_AUDIO_REPORT_ID0:
     *   data[0]: 0xF7; data[1]: KeyCode LSB;data[2]: KeyCode MSB;
     *   data[3]: SequenceNumber; data[4]: mSBC SyncWord, data[5...]: Voice Data
     * APP_HH_GOOGLE_AUDIO_REPORT_ID1:
     *   data[0]: 0xFA; data[1...]: Voice Data
     * APP_HH_GOOGLE_AUDIO_REPORT_ID2:
     *   data[0]: 0xFB; data[1...]: Voice Data
     */

    if (report_id == APP_HH_GOOGLE_AUDIO_REPORT_ID0)
    {
        keyCode = *((UINT16*)&p_data[1]);
        seqNum = p_data[3];
        memcpy(&dataBuf[2], &p_data[3], length - 3);
        p_data = dataBuf;
    }
#endif
...
#if defined(GOOGLE_ALT_AUDIO_FORMAT) && (GOOGLE_ALT_AUDIO_FORMAT == TRUE)
        if((report_id == APP_HH_GOOGLE_AUDIO_REPORT_ID0) && (app_hh_ble_audio_sink_cb.startSeq == FALSE))
        {
            printf("Got start sequence\n");
            app_hh_ble_audio_sink_cb.startSeq = TRUE;
            app_hh_ble_audio_sink_cb.bytesInFrm = 0;
            app_hh_ble_audio_sink_cb.pkt_cnt = 0;
        }
#else
    ...
    if ((app_hh_ble_audio_sink_cb.startFrm == TRUE) && (app_hh_ble_audio_sink_cb.startSeq == TRUE))
    {
        ...
        {
            copySize = APP_HH_AUDIO_DATA_SIZE0;
        ...
        {
            copySize = APP_HH_AUDIO_DATA_SIZE1;
        ...
        {
            copySize = APP_HH_AUDIO_DATA_SIZE2;
            memcpy(&app_hh_ble_audio_sink_cb.inFrm[app_hh_ble_audio_sink_cb.bytesInFrm], &p_data[1], copySize);
            app_hh_ble_audio_sink_cb.bytesInFrm += copySize;
        }
        else
