...
#if defined(PCM_OPUS) && (PCM_OPUS == TRUE)
#include "app_hh_opus.h"
#endif

#if defined(PCM_ADPCM) && (PCM_ADPCM == TRUE)
#include "app_hh_adpcm.h"
#endif
 ...
 * Defines
 */
const UINT8 google_voice_recognition_start[] = {0xF8, 0x0C, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
const UINT8 google_voice_recognition_start_len = sizeof(google_voice_recognition_start);

const UINT8 google_voice_recognition_stop[] = {0xF8, 0x0D, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
const UINT8 google_voice_recognition_stop_len = sizeof(google_voice_recognition_stop);
...
#if defined(APP_HH_BLE_AUDIO_INCLUDED) && (APP_HH_BLE_AUDIO_INCLUDED == TRUE)
#if defined(GOOGLE_ALT_AUDIO_FORMAT) && (GOOGLE_ALT_AUDIO_FORMAT == TRUE)
    if ((p_hh_dev != NULL) &&
        (p_hh_dev->audio.is_ble_audio_device) &&
        (!p_hh_dev->audio.is_audio_active) &&
        (p_uipc_report->report.report_data.data[0] == 0x02) &&
        (p_uipc_report->report.report_data.data[1] == 0x00) &&
        (p_uipc_report->report.report_data.data[2] == 0x01))
    {
        tBSA_HH_REPORT_DATA hid;
        hid.length = google_voice_recognition_start_len;
        memcpy(hid.data, google_voice_recognition_start, google_voice_recognition_start_len);

        APP_DEBUG0("got mic key down, fake to audio start");
        app_hh_ble_audio_sink_handler(p_hh_dev, &hid);
        GKI_freebuf(p_msg);
        return;
    }
    else if ((p_hh_dev != NULL) &&
        (p_hh_dev->audio.is_ble_audio_device) &&
        (p_hh_dev->audio.is_audio_active) &&
        (p_uipc_report->report.report_data.data[0] == 0x02) &&
        (p_uipc_report->report.report_data.data[1] == 0x00) &&
        (p_uipc_report->report.report_data.data[2] == 0x00))
    {
        tBSA_HH_REPORT_DATA hid;
        hid.length = google_voice_recognition_stop_len;
        memcpy(hid.data, google_voice_recognition_stop, google_voice_recognition_stop_len);

        APP_DEBUG0("got mic key up, terminate voice");
        app_hh_ble_audio_sink_handler(p_hh_dev, &hid);
        GKI_freebuf(p_msg);
        return;
    }
...
#if defined(GOOGLE_ALT_AUDIO_FORMAT) && (GOOGLE_ALT_AUDIO_FORMAT == TRUE)
        ((p_uipc_report->report.report_data.data[0] == APP_HH_GOOGLE_AUDIO_REPORT_ID0) ||
          (p_uipc_report->report.report_data.data[0] == APP_HH_GOOGLE_AUDIO_REPORT_ID1) ||
          (p_uipc_report->report.report_data.data[0] == APP_HH_GOOGLE_AUDIO_REPORT_ID2) ||
#else
        ((p_uipc_report->report.report_data.data[0] == APP_HH_AUDIO_REPORT_ID) ||
#endif
...
int app_hh_init(void)
{
    ...
    for (index = 0 ; index < APP_NUM_ELEMENTS(app_hh_cb.devices); index++)
    {
        ...
        p_hh_dev->audio.format.sample_rate = APP_HH_AUDIO_SAMPLE_RATE;
