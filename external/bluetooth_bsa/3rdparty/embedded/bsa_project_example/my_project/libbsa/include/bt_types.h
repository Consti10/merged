...
#define BT_EVT_TO_BTU_HCI_CMD       0x1600      /* HCI command from upper
                                                   layer */


#define BT_EVT_TO_BTU_L2C_SEG_XMIT  0x1900      /* L2CAP segment(s)
                                                   transmitted */

#define BT_EVT_PROXY_INCOMING_MSG   0x1A00      /* BlueStackTester event:
                                                   incoming message from
                                                   target */

#define BT_EVT_BTSIM                0x1B00      /* Insight BTSIM event */
#define BT_EVT_BTISE                0x1C00      /* Insight Script Engine
                                                   event */
...
#define BT_EVT_TO_LM_LC_LMP         0x2500      /* LC Received LMP command
                                                   frame                      */
...
#define BT_EVT_TO_LM_LC_LMPC_TX     0x2900      /* LMP Command transmit
                                                   complete                   */
#define BT_EVT_TO_LM_LOCAL_ACL_LB   0x2a00      /* Data to be locally
                                                   loopbacked                 */
...
/* for NFC                          */
                                                /******************************/
#define BT_EVT_TO_NFC_NCI           0x4000      /* NCI Command, Notification
                                                   or Data                    */
#define BT_EVT_TO_NFC_NCI_VS        0x4200      /* Vendor specific message    */
#define BT_EVT_TO_NFC_MSGS          0x4300      /* messages between NFC and
                                                   NCI task                   */
#define BT_EVT_TO_LLCP_MSGS         0x4400      /* messages for LLCP          */
#define BT_EVT_TO_NFCCSIM_NCI       0x4a00      /* events to NFCC simulation
                                                   (NCI packets)              */
...
#define BT_PSM_BNEP                     0x000F /* Bluetooth Network
                                                  Encapsulation Protocol */
#define BT_PSM_HIDC                     0x0011 /* Human Interface Device
                                                  (Control) */
#define BT_PSM_HIDI                     0x0013 /* Human Interface Device
                                                  (Interrupt) */
#define BT_PSM_UPNP                     0x0015 /* Universal PlugNPlay */
#define BT_PSM_AVCTP                    0x0017 /* Audio/Video Control
                                                  Transport Protocol */
#define BT_PSM_AVDTP                    0x0019 /* Audio/Video Distribution
                                                  Transport Protocol */
#define BT_PSM_AVCTP_13                 0x001B /* Advanced Control - Browsing */
#define BT_PSM_UDI_CP                   0x001D /* Unrestricted Digital
                                                  Information C-Plane  */
#define BT_PSM_ATT                      0x001F /* Attribute Protocol  */
#define BT_PSM_3DS                      0x0021 /* 3D sync */
#define BT_LE_PSM_IPSP                  0x0023 /* Internet Protocol Support
                                                  (IPv6/6LoWPAN) */


/* These macros extract the HCI opcodes from a buffer
*/
#define HCI_GET_CMD_HDR_OPCODE(p)    (UINT16)((*((UINT8 *)((p) + 1) + \
                                     p->offset) + \
                                     (*((UINT8 *)((p) + 1) + \
                                     p->offset + 1) << 8)))
#define HCI_GET_CMD_HDR_PARAM_LEN(p) (UINT8)  (*((UINT8 *)((p) + 1) + \
                                     p->offset + 2))

#define HCI_GET_EVT_HDR_OPCODE(p)    (UINT8)(*((UINT8 *)((p) + 1) + p->offset))
#define HCI_GET_EVT_HDR_PARAM_LEN(p) (UINT8)  (*((UINT8 *)((p) + 1) + \
                                     p->offset + 1))
...
*/
#define UINT128_TO_STREAM(p, u128) {*(p)++ = (UINT8)(u128);         \
                                    *(p)++ = (UINT8)((u128) >> 8);  \
                                    *(p)++ = (UINT8)((u128) >> 16); \
                                    *(p)++ = (UINT8)((u128) >> 24); \
                                    *(p)++ = (UINT8)((u128) >> 32); \
...
#define UINT64_TO_STREAM(p, u64) {*(p)++ = (UINT8)(u64);            \
                                  *(p)++ = (UINT8)((u64) >> 8);     \
                                  *(p)++ = (UINT8)((u64) >> 16);    \
                                  *(p)++ = (UINT8)((u64) >> 24);    \
                                  *(p)++ = (UINT8)((u64) >> 32);    \
...
#define UINT48_TO_STREAM(p, u48) {*(p)++ = (UINT8)(u48);            \
                                  *(p)++ = (UINT8)((u48) >> 8);     \
                                  *(p)++ = (UINT8)((u48) >> 16);    \
                                  *(p)++ = (UINT8)((u48) >> 24);    \
                                  *(p)++ = (UINT8)((u48) >> 32);    \
...
#define UINT40_TO_STREAM(p, u40) {*(p)++ = (UINT8)(u40);            \
                                  *(p)++ = (UINT8)((u40) >> 8);     \
                                  *(p)++ = (UINT8)((u40) >> 16);    \
                                  *(p)++ = (UINT8)((u40) >> 24);    \
                                  *(p)++ = (UINT8)((u40) >> 32);}

#define UINT32_TO_STREAM(p, u32) {*(p)++ = (UINT8)(u32);            \
                                  *(p)++ = (UINT8)((u32) >> 8);     \
                                  *(p)++ = (UINT8)((u32) >> 16);    \
                                  *(p)++ = (UINT8)((u32) >> 24);}

#define UINT24_TO_STREAM(p, u24) {*(p)++ = (UINT8)(u24);            \
                                  *(p)++ = (UINT8)((u24) >> 8);     \
                                  *(p)++ = (UINT8)((u24) >> 16);}

#define UINT16_TO_STREAM(p, u16) {*(p)++ = (UINT8)(u16);            \
                                  *(p)++ = (UINT8)((u16) >> 8);}

#define UINT8_TO_STREAM(p, u8)   {*(p)++ = (UINT8)(u8);}
#define INT8_TO_STREAM(p, u8)    {*(p)++ = (INT8)(u8);}
#define ARRAY32_TO_STREAM(p, a)  {register int ijk;                 \
                                  for (ijk = 0; ijk < 32; ijk++)    \
                                  *(p)++ = (UINT8) a[31 - ijk];}

#define ARRAY16_TO_STREAM(p, a)  {register int ijk;                 \
                                  for (ijk = 0; ijk < 16; ijk++)    \
                                  *(p)++ = (UINT8) a[15 - ijk];}

#define ARRAY8_TO_STREAM(p, a)   {register int ijk;                 \
                                  for (ijk = 0; ijk < 8; ijk++)     \
                                  *(p)++ = (UINT8) a[7 - ijk];}

#define BDADDR_TO_STREAM(p, a)   {register int ijk;                       \
                                  for (ijk = 0; ijk < BD_ADDR_LEN; ijk++) \
                                  *(p)++ = (UINT8) a[BD_ADDR_LEN - 1 - ijk];}
#define LAP_TO_STREAM(p, a)      {register int ijk;                   \
                                  for (ijk = 0; ijk < LAP_LEN; ijk++) \
                                  *(p)++ = (UINT8) a[LAP_LEN - 1 - ijk];}

#define DEVCLASS_TO_STREAM(p, a) {register int ijk;                        \
                                  for (ijk = 0; ijk < DEV_CLASS_LEN;ijk++) \
                                  *(p)++ = (UINT8) a[DEV_CLASS_LEN - 1 - ijk];}

#define ARRAY_TO_STREAM(p, a, len) {register int ijk;               \
                                    for (ijk = 0; ijk < len; ijk++) \
                                    *(p)++ = (UINT8) a[ijk];}

#define REVERSE_ARRAY_TO_STREAM(p, a, len)  {register int ijk;               \
                                             for (ijk = 0; ijk < len; ijk++) \
                                             *(p)++ = (UINT8) a[len - 1 - ijk];}

#define STREAM_TO_UINT8(u8, p)   {u8 = (UINT8)(*(p)); (p) += 1;}

#define STREAM_TO_UINT16(u16, p) {u16 = ((UINT16)(*(p)) +           \
                                 (((UINT16)(*((p) + 1))) << 8));    \
                                 (p) += 2;}

#define STREAM_TO_UINT24(u32, p) {u32 = (((UINT32)(*(p))) +         \
                                 ((((UINT32)(*((p) + 1)))) << 8) +  \
                                 ((((UINT32)(*((p) + 2)))) << 16) );\
                                 (p) += 3;}

#define STREAM_TO_UINT32(u32, p) {u32 = (((UINT32)(*(p))) +         \
                                 ((((UINT32)(*((p) + 1)))) << 8) +  \
                                 ((((UINT32)(*((p) + 2)))) << 16) + \
                                 ((((UINT32)(*((p) + 3)))) << 24)); \
                                 (p) += 4;}

#define STREAM_TO_UINT40(u40, p) {u40 = (((UINT64)(*(p))) +         \
                                 ((((UINT64)(*((p) + 1)))) << 8) +  \
                                 ((((UINT64)(*((p) + 2)))) << 16) + \
                                 ((((UINT64)(*((p) + 3)))) << 24) + \
                                 ((((UINT64)(*((p) + 4)))) << 32)); \
...
#define STREAM_TO_BDADDR(a, p)   {register int ijk; \
                                  register UINT8 *pbda = (UINT8 *)a + \
                                           BD_ADDR_LEN - 1;           \
                                  for (ijk = 0; ijk < BD_ADDR_LEN; ijk++) \
                                  *pbda-- = *p++;}

#define STREAM_TO_ARRAY32(a, p)  {register int ijk; \
                                  register UINT8 *_pa = (UINT8 *)a + 31; \
                                  for (ijk = 0; ijk < 32; ijk++) \
                                  *_pa-- = *p++;}

#define STREAM_TO_ARRAY16(a, p)  {register int ijk; \
                                  register UINT8 *_pa = (UINT8 *)a + 15; \
                                  for (ijk = 0; ijk < 16; ijk++) \
                                  *_pa-- = *p++;}

#define STREAM_TO_ARRAY8(a, p)   {register int ijk; \
                                  register UINT8 *_pa = (UINT8 *)a + 7; \
                                  for (ijk = 0; ijk < 8; ijk++) \
                                  *_pa-- = *p++;}

#define STREAM_TO_DEVCLASS(a, p) {register int ijk; \
                                  register UINT8 *_pa = (UINT8 *)a + \
                                           DEV_CLASS_LEN - 1; \
                                  for (ijk = 0; ijk < DEV_CLASS_LEN; ijk++) \
                                  *_pa-- = *p++;}

#define STREAM_TO_LAP(a, p)      {register int ijk; \
                                  register UINT8 *plap = (UINT8 *)a + \
                                           LAP_LEN - 1; \
                                  for (ijk = 0; ijk < LAP_LEN; ijk++) \
                                  *plap-- = *p++;}

#define STREAM_TO_ARRAY(a, p, len) {register int ijk; \
                                    for (ijk = 0; ijk < len; ijk++) \
                                    ((UINT8 *) a)[ijk] = *p++;}

#define REVERSE_STREAM_TO_ARRAY(a, p, len) {register int ijk; \
                                            register UINT8 *_pa = (UINT8 *)a + \
                                                     len - 1; \
                                            for (ijk = 0; ijk < len; ijk++) \
                                            *_pa-- = *p++;}
...
*/
#define UINT32_TO_FIELD(p, u32) {*(UINT8 *)(p) = (UINT8)(u32); \
                                 *((UINT8 *)(p)+1) = (UINT8)((u32) >> 8); \
                                 *((UINT8 *)(p)+2) = (UINT8)((u32) >> 16); \
                                 *((UINT8 *)(p)+3) = (UINT8)((u32) >> 24);}

#define UINT24_TO_FIELD(p, u24) {*(UINT8 *)(p) = (UINT8)(u24); \
                                 *((UINT8 *)(p)+1) = (UINT8)((u24) >> 8); \
                                 *((UINT8 *)(p)+2) = (UINT8)((u24) >> 16);}

#define UINT16_TO_FIELD(p, u16) {*(UINT8 *)(p) = (UINT8)(u16); \
                                 *((UINT8 *)(p)+1) = (UINT8)((u16) >> 8);}
...
*/
#define UINT128_TO_BE_STREAM(p, u128) {*(p)++ = (UINT8)(u128)>> 120; \
                                       *(p)++ = (UINT8)((u128) >> 112);\
                                       *(p)++ = (UINT8)((u128) >> 104);\
                                       *(p)++ = (UINT8)((u128) >> 96); \
                                       *(p)++ = (UINT8)((u128) >> 88); \
...
#define UINT64_TO_BE_STREAM(p, u64) {*(p)++ = (UINT8)((u64) >> 56); \
                                     *(p)++ = (UINT8)((u64) >> 48); \
                                     *(p)++ = (UINT8)((u64) >> 40); \
                                     *(p)++ = (UINT8)((u64) >> 32); \
                                     *(p)++ = (UINT8)((u64) >> 24); \
...
#define UINT32_TO_BE_STREAM(p, u32) {*(p)++ = (UINT8)((u32) >> 24); \
                                     *(p)++ = (UINT8)((u32) >> 16); \
                                     *(p)++ = (UINT8)((u32) >> 8);  \
                                     *(p)++ = (UINT8)(u32); }

#define UINT24_TO_BE_STREAM(p, u24) {*(p)++ = (UINT8)((u24) >> 16); \
                                     *(p)++ = (UINT8)((u24) >> 8);  \
                                     *(p)++ = (UINT8)(u24);}

#define UINT16_TO_BE_STREAM(p, u16) {*(p)++ = (UINT8)((u16) >> 8);  \
                                     *(p)++ = (UINT8)(u16);}

#define UINT8_TO_BE_STREAM(p, u8)   {*(p)++ = (UINT8)(u8);}
#define ARRAY_TO_BE_STREAM(p, a, len) {register int ijk; \
                                       for (ijk = 0; ijk < len; ijk++) \
                                       *(p)++ = (UINT8) a[ijk];}

#define BE_STREAM_TO_UINT8(u8, p)   {u8 = (UINT8)(*(p)); (p) += 1;}
#define BE_STREAM_TO_UINT16(u16, p) {u16 = (UINT16)(((UINT16)(*(p)) << 8) + \
                                    (UINT16)(*((p) + 1))); (p) += 2;}

#define BE_STREAM_TO_UINT24(u32, p) {u32 = (((UINT32)(*((p) + 2))) + \
                                    ((UINT32)(*((p) + 1)) << 8) + \
                                    ((UINT32)(*(p)) << 16)); (p) += 3;}

#define BE_STREAM_TO_UINT32(u32, p) {u32 = ((UINT32)(*((p) + 3)) + \
                                    ((UINT32)(*((p) + 2)) << 8) + \
                                    ((UINT32)(*((p) + 1)) << 16) + \
                                    ((UINT32)(*(p)) << 24)); (p) += 4;}

#define BE_STREAM_TO_UINT64(u64, p) {u64 = ((UINT64)(*((p) + 7)) + \
                                    ((UINT64)(*((p) + 6)) << 8) + \
                                    ((UINT64)(*((p) + 5)) << 16) + \
                                    ((UINT64)(*((p) + 4)) << 24) + \
                                    ((UINT64)(*((p) + 3)) << 32) + \
...
#define BE_STREAM_TO_ARRAY(p, a, len) {register int ijk; \
                                       for (ijk = 0; ijk < len; ijk++) \
                                       ((UINT8 *) a)[ijk] = *p++;}
...
** These are the same as to stream, except the pointer is not incremented.
*/
#define UINT32_TO_BE_FIELD(p, u32) {*(UINT8 *)(p) = (UINT8)((u32) >> 24);  \
                                    *((UINT8 *)(p)+1) = (UINT8)((u32) >> 16); \
                                    *((UINT8 *)(p)+2) = (UINT8)((u32) >> 8); \
                                    *((UINT8 *)(p)+3) = (UINT8)(u32); }

#define UINT24_TO_BE_FIELD(p, u24) {*(UINT8 *)(p) = (UINT8)((u24) >> 16); \
                                    *((UINT8 *)(p)+1) = (UINT8)((u24) >> 8); \
                                    *((UINT8 *)(p)+2) = (UINT8)(u24);}

#define UINT16_TO_BE_FIELD(p, u16) {*(UINT8 *)(p) = (UINT8)((u16) >> 8); \
                                    *((UINT8 *)(p)+1) = (UINT8)(u16);}
...
typedef UINT8 PIN_CODE[PIN_CODE_LEN];       /* Pin Code (upto 128 bits) MSB
                                               is 0 */
...
typedef UINT8 *EXT_INQ_RESP_PTR;             /* Pointer to Extended Inquiry
                                                Response */
...
typedef UINT8 BD_FEATURES[BD_FEATURES_LEN]; /* LMP features supported by
                                               device */
...
#define SYNTH_TX                1           /* want synth code to TRANSMIT at
                                               this freq */
#define SYNTH_RX                2           /* want synth code to RECEIVE at
                                               this freq */

#define SYNC_REPS 1             /* repeats of sync word transmitted to start
                                   of burst */
...
*/
#define BRCM_PROPRIETARY_UUID_BASE  0xDA, 0x23, 0x41, 0x02, 0xA3, 0xBB, 0xC1, \
                                    0x71, 0xBA, 0x09, 0x6f, 0x21
#define BRCM_PROPRIETARY_GUID_BASE  0xda23, 0x4102, 0xa3, 0xbb, 0xc1, 0x71, \
                                    0xba, 0x09, 0x6f, 0x21
...
/* Define trace levels */
#define BT_TRACE_LEVEL_NONE     0          /* No trace messages to be
                                              generated                       */
#define BT_TRACE_LEVEL_ERROR    1          /* Error condition trace messages  */
#define BT_TRACE_LEVEL_WARNING  2          /* Warning condition trace
                                              messages                        */
#define BT_TRACE_LEVEL_CUSTOM   3          /* Custom condition trace
                                              messages                        */
...
#define TRACE_LAYER_GKI             0x00160000 /* it's overwritten in
                                                  nfc_types.h*/
...
#define TRACE_LAYER_NCI             0x00230000 /* it's overwritten in
                                                  nfc_types.h */
...
#define TRACE_LAYER_HAL             0x002c0000 /* it's overwritten in
                                                  nfc_types.h */
...
#define SCR_PROTO_TRACE_HCI_LOGGING_VSE 0x0800 /* Brcm vs event for logmsg
                                                  and protocol traces */
