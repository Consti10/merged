...
export class PlayerComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  setPlay(url: string, streamId: number = -1) {
    ...
    if (streamId >= 0) {
      ...
      if (this.playerUrlList[streamId].format == 'H.265') {
        this.httpFlvUnsupportIntervalFunc();
        this.logger.log('unsupport h265');
        return;
      }
    }
  ...
  httpFlvPlay(palyUrl: string, isH265: boolean = false) {
    ...
    const options: any = {
      ...
      bufferingTime: 0, // 缓冲时间，与延时有关
      cacheSegmentCount: 128, // 内部最大的缓存帧数量，默认256，太小会一直在playing和buffering中切换，导致卡顿
      customLoader: null,
    };
